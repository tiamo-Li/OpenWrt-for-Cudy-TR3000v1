...
# OpenWrt多SSID专业配置脚本
# 检查参数
if [ -z "$1" ] || ! [ "$1" -eq "$1" ] 2>/dev/null || [ "$1" -lt 1 ]; then
...
# 设置passwall远程DNS、启用passwall访问控制
uci set passwall.cfg013fd6.remote_dns='8.8.8.8'
uci set "$APPNAME.@global[0].acl_enable=1"
uci del passwall.cfg013fd6.tcp_node_socks_port
uci del passwall.cfg013fd6.use_default_dns
uci del passwall.cfg013fd6.use_direct_list
uci del passwall.cfg013fd6.use_proxy_list
uci del passwall.cfg013fd6.use_block_list
uci del passwall.cfg013fd6.use_gfw_list
...
for i in $(seq 1 $SSID_COUNT); do
    ...
    # 创建ACL规则段落
    uci set "$APPNAME.$rule_id=acl_rule"
    uci set "$APPNAME.$rule_id.enabled=1"
    uci set "$APPNAME.$rule_id.remarks=$remark"
    uci set "$APPNAME.$rule_id.sources=$source"
    uci set "$APPNAME.$rule_id.interface=$remark"
    uci set "$APPNAME.$rule_id.use_global_config=1"
    ...
    uci set wireless.$section.ssid="5G_$interface_num"
    ...
    uci set network.$interface_num.ipaddr="192.168.$ip_last.1"
