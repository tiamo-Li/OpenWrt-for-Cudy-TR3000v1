...
for i in $(seq 1 $SSID_COUNT); do
    ...
    uci set $APPNAME.$rule_id.use_global_config='0'
    uci set $APPNAME.$rule_id.tcp_node='myshunt'
    uci set $APPNAME.$rule_id.use_gfw_list='0'
    uci set $APPNAME.$rule_id.chn_list='0'
    uci set $APPNAME.$rule_id.tcp_proxy_mode='proxy'
    uci set $APPNAME.$rule_id.dns_shunt='dnsmasq'
    uci set $APPNAME.$rule_id.dns_mode='dns2socks'
    uci set $APPNAME.$rule_id.remote_dns='8.8.8.8'
    uci set $APPNAME.$rule_id.udp_node='tcp'
    uci set $APPNAME.$rule_id.udp_proxy_mode='proxy'
